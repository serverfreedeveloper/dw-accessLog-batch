...
/**
 * 導管管理画面（導管タブ）CSV出力時アクセスログ送信用Service
 */
@Slf4j
@RequiredArgsConstructor
@Service
public class DwUketsukeMngSendLogService {
  ...
  /** コンテナ名（導管管理画面（導管タブ）） */
  @Value("${DW_UKETUKE_MNG_APP_NAME}")
  private String containerName;

  public void exec() {

    log.info("==================== アクセスログ送信 導管タブ 処理開始 ====================");
    ...
    try {
      ...
      log.warn("アクセスログ 導管タブ 送信処理に失敗しました。", e.getMessage());
    } finally {
      log.info("==================== アクセスログ送信 導管タブ 処理終了 ====================");
    }
  }
}
